/**
 * Facts & Fun settings — per-grade complexity lookup and admin updates.
 */
...
async function getComplexityForGrade(grade) {
  ...
  try {
    const r = await pool.query(
      `SELECT complexity, enabled FROM daily_facts_settings WHERE grade = $1`,
      [key],
    );
    if (r.rows.length > 0) {
      const row = r.rows[0];
    ...
    console.warn('[dailyFactsSettings] lookup failed:', err.message);
  }
...
async function updateSettings(updates, userId) {
  ...
  for (const item of updates) {
    ...
    await pool.query(
      `INSERT INTO daily_facts_settings (grade, complexity, enabled, updated_by, updated_at)
