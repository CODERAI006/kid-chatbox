 ...
 * Interactive flashcard deck — flip, navigate, shuffle, and track mastery.
...
export function InteractiveFlashcardDeck({
  ...
  const faceText = flipped ? current?.back : current?.front;
  ...
  return (
    ...
    <VStack align="stretch" spacing={compact ? 2 : 3}>
      ...
      <Box
        ...
        minH={compact ? '100px' : '120px'}

        borderRadius="xl"

        borderWidth={2}

        borderColor={flipped ? 'purple.400' : 'blue.300'}

        bg={flipped ? 'purple.50' : 'blue.50'}
      ...
      >
        ...
        <Badge mb={2} colorScheme={flipped ? 'purple' : 'blue'}>

          {flipped ? 'Answer' : 'Question'}

        </Badge>

        <Text fontSize={compact ? 'sm' : 'md'} fontWeight="semibold" lineHeight="tall" whiteSpace="pre-wrap">

          {faceText || (flipped ? 'Answer missing for this card.' : 'Question missing for this card.')}

        </Text>

        <Text fontSize="xs" color="gray.500" mt={2}>

          Tap to {flipped ? 'see question' : 'see answer'}
